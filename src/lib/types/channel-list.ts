...
export function validateChannelList(channelList: any): ChannelListValidationResult {
  ...
  if (!channelList.ownerId || typeof channelList.ownerId !== 'string' || channelList.ownerId.trim() === '') {
    errors.push({
      field: 'ownerId',
  ...
  if (typeof channelList.channelCount !== 'number' || channelList.channelCount < 0 || !Number.isInteger(channelList.channelCount)) {
    errors.push({
      field: 'channelCount',
      message: 'Channel count must be a non-negative integer'
    });
  } else if (Array.isArray(channelList.channelIds) && channelList.channelCount !== channelList.channelIds.length) {
    errors.push({
      field: 'channelCount',
  ...
  if (channelList.description !== undefined && (typeof channelList.description !== 'string' || channelList.description.length > 500)) {
    errors.push({
      field: 'description',
  ...
  if (channelList.ownerId && typeof channelList.ownerId === 'string' && channelList.ownerId.length < 28) {
    errors.push({
      field: 'ownerId',
 ...
 */
export function removeChannelsFromList(channelList: ChannelList, channelIdsToRemove: string[]): ChannelList {
